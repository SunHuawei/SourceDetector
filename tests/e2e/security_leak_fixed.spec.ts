...
test('detects security leak and shows OpenAI warning in popup', async ({ context, serviceWorker }) => {
  ...
  // Directly trigger source map detection via background message
  await serviceWorker.evaluate(async () => {
    const mockPageUrl = 'http://127.0.0.1:4173/index.html';
    const mockSourceUrl = 'http://127.0.0.1:4173/app.bundle.js';
    const mockMapUrl = 'http://127.0.0.1:4173/app.bundle.js.map';
    
    await chrome.runtime.sendMessage({
      type: 'FOUND_SOURCE_MAP',
      data: {
        pageTitle: 'SourceDetector Mock Site',
        pageUrl: mockPageUrl,
        sourceUrl: mockSourceUrl,
        mapUrl: mockMapUrl,
        fileType: 'js',
        originalContent: "const mockOpenAiKey = 'sk-1234567890abcdef1234567890abcdef1234567890abcdef';"
      }
    });
  });
        ...
        detectionState = await serviceWorker.evaluate(async (): Promise<LeakDetectionState | null> => {
          ...
          const pageDataResponse = await new Promise<any>((resolve) => {
            chrome.runtime.sendMessage(
              { type: 'GET_PAGE_DATA', data: { url: activeTab.url } },
              (response) => resolve(response)
            );
          });

          const files = Array.isArray(pageDataResponse?.data?.files)
            ? pageDataResponse.data.files
            : [];
          ...
          return { badgeText, badgeColor, fileCount: files.length, findingCount, fileNames };
        });
        return Boolean(
          detectionState &&
