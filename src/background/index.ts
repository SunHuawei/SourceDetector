...
import { AppSettings, CrxFile, PageData, ResolvedPageContext } from '@/types';
...
const pendingCrxHydration = new Map<string, Promise<CrxFile | null>>();

function hasCrxPayload(crxFile: CrxFile | null | undefined): crxFile is CrxFile {
  return Boolean(crxFile && crxFile.blob instanceof Blob && crxFile.blob.size > 0);
}
...
interface EnsureCrxFileOptions {
  pageUrl: string;
  pageTitle: string;
  crxUrl: string;
  existingCrxFile?: CrxFile;
}

async function ensureCrxFile(
  options: EnsureCrxFileOptions
): Promise<CrxFile | null> {
  const { pageUrl, pageTitle, crxUrl, existingCrxFile } = options;
  const cacheKey = existingCrxFile ? `id:${existingCrxFile.id}` : `url:${pageUrl}|${crxUrl}`;
  const usableExistingCrxFile = hasCrxPayload(existingCrxFile) ? existingCrxFile : null;

  const pendingTask = pendingCrxHydration.get(cacheKey);
  if (pendingTask) {
    return pendingTask;
  }
  ...
  const task = (async () => {
    try {
      ...
      if (!result) {
        return usableExistingCrxFile;
      }

      const contentHash = await createBlobHash(result.blob);
      const nextRecord = {
        pageUrl,
        pageTitle,
        crxUrl,
      ...
      if (existingCrxFile) {
        const nextCrxFile: CrxFile = {
          ...existingCrxFile,
          ...nextRecord,
        };
        await db.updateCrxFile(nextCrxFile);
        return nextCrxFile;
      }

      const createdCrxFile = await db.addCrxFile(nextRecord);
      return createdCrxFile;
    } catch (error) {
      console.error('Error ensuring CRX file payload:', error);
      return usableExistingCrxFile;
    }
  })();

  pendingCrxHydration.set(cacheKey, task);

  try {
    return await task;
  } finally {
    pendingCrxHydration.delete(cacheKey);
  }
}
...
async function updateCrxPage(tab: browserAPI.tabs.Tab) {
  ...
  const existingCrxFile = await db.getCrxFileByPageUrl(normalizedUrl);
  const resolvedCrxFile = await ensureCrxFile({
    pageUrl: normalizedUrl,
    pageTitle: tab.title || existingCrxFile?.pageTitle || '',
    crxUrl,
    existingCrxFile,
  });
...
async function fetchSourceMapContent(
  sourceUrl: string,
  mapUrl: string
): Promise<{
  content: string;
...
async function resolveRequestPageContext(
  details: RequestPageContextDetails
...
async function handleGetCrxFile(data: { url: string }) {
  try {
    const lookupUrl = typeof data.url === 'string' ? data.url : '';
    const normalizedLookupUrl = normalizePageUrl(lookupUrl);
    let crxFile =
      (await db.getCrxFileByPageUrl(normalizedLookupUrl)) ??
      (await db.getCrxFileByCrxUrl(lookupUrl)) ??
      (await db.getCrxFileByCrxUrl(normalizedLookupUrl));

    if (!hasCrxPayload(crxFile)) {
      const pageUrl = crxFile?.pageUrl ?? normalizedLookupUrl;
      const pageTitle = crxFile?.pageTitle ?? '';
      const crxUrl = crxFile?.crxUrl ?? (await getCrxUrl(pageUrl));

      if (crxUrl) {
        const ensuredCrxFile = await ensureCrxFile({
          pageUrl,
          pageTitle,
          crxUrl,
          existingCrxFile: crxFile,
        ...
        if (ensuredCrxFile) {
          crxFile = ensuredCrxFile;
        }
      }
    }

    if (!crxFile) {
      return { success: false, reason: 'CRX file not found' };
    }
...
async function handleGetPageData(data: { url: string }) {
  try {
    ...
    const files = await db.getPageFileSummaries(normalizedUrl);
...
async function handleGetFileData(data: { id?: number; url?: string }) {
  try {
    const file =
      typeof data.id === 'number'
        ? await db.sourceMapFiles.get(data.id)
        : typeof data.url === 'string'
          ? await db.sourceMapFiles.where('url').equals(data.url).first()
          : undefined;
...
async function handleSourceMapFound(data: {
  pageTitle: string;
  pageUrl: string;
  sourceUrl: string;
  mapUrl: string;
  ...
  return taskQueue.enqueue('sourceMap', async () => {
    try {
      ...
      if (existingFile) {
        const existingFiles = await db.sourceMapFiles.where('url').equals(data.sourceUrl).toArray();
        await Promise.all(
          existingFiles.map(file => db.sourceMapFiles.update(file.id, { isLatest: false }))
        );
      }

      // Get latest version number
      const latestVersion = existingFile
        ? (await db.sourceMapFiles.where('url').equals(data.sourceUrl).toArray()).reduce(
...
async function handleDeleteSourceMap(data: { url: string }) {
  try {
    await db.sourceMapFiles.where('url').equals(data.url).delete();
...
async function handleClearHistory() {
  try {
    await Promise.all([db.sourceMapFiles.clear(), db.pages.clear(), db.pageSourceMaps.clear()]);
    return { success: true };
  } catch (error) {
...
async function checkAndCleanStorage() {
  try {
    ...
    if (stats.totalSize > settings.cleanupThreshold * 1024 * 1024) {
      ...
      await db.sourceMapFiles.orderBy('timestamp').limit(100).delete();
    }
  } catch (error) {
...
async function getCrxUrl(url: string): Promise<string | null> {
  try {
    ...
    const cws_pattern =
      /^https?:\/\/(?:chrome.google.com\/webstore|chromewebstore.google.com)\/.+?\/([a-z]{32})(?=[\/#?]|$)/;
    const match = cws_pattern.exec(url);
    const extId =
      match?.[1] || url.split('/')[6]?.split('?')[0] || url.split('//')[1]?.split('/')[0];
